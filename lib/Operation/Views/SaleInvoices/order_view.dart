...
class _CheckOutState extends State<CheckOut> {
  ...
  GetBuilder<Sale_Invoices_Controller> MAT_FOL_BUILDER(MGNO, MINO) {
    return GetBuilder<Sale_Invoices_Controller>(
        init: Sale_Invoices_Controller(),
        builder: ((controller) =>
            FutureBuilder<List<Mat_Fol_Local>>(
              future: GET_MAT_FOL(
                controller.SelectDataBIID.toString(), MGNO, MINO,),
              builder: (BuildContext context,
                  AsyncSnapshot<List<Mat_Fol_Local>> snapshot) {
                      ...
                      return Padding(
                        ...
                        child: Container(
                          ...
                          Expanded(child: AnimatedTextWidget(
                              text: "${item.MINA_D.toString()} (${controller
                                  .formatter.format(item.MFNOF)})")) :
                          Text(
                            "${item.MINA_D} (${controller.formatter.format(
                                item.MFNOF)})",
                            style: ThemeHelper().buildTextStyle(
                              context, Colors.black, 'M',
                            ),
  ...
  GetBuilder<Sale_Invoices_Controller> MAT_FOL_BULDER(double height, MGNO, MINO,
      String BMMID, String BMDID) {
    return GetBuilder<Sale_Invoices_Controller>(
        init: Sale_Invoices_Controller(),
        builder: ((controller) =>
            FutureBuilder<List<Mat_Des_M_Local>>(
              future: GET_MAT_DES_M(MGNO, MINO),
              builder: (BuildContext context,
                  AsyncSnapshot<List<Mat_Des_M_Local>> snapshot) {
                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                  return Container(); // إرجاع عنصر فارغ إذا لم توجد بيانات
                }
                ...
                return SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      final item = snapshot.data![index];
                      final isSelected = controller.detailsMap[currentKey]!
                          .contains(item.MDMID.toString());
                      return Padding(
                          ...
                          onTap: () async {
                            try {
                              if (isSelected) {
                                ...
                                controller.detailsMap[currentKey]!.remove(
                                    item.MDMID.toString());
                              } else {
                                // إضافة العنصر إلى القائمة إذا لم يكن محددًا
                                controller.detailsMap[currentKey]!.add(
                                    item.MDMID.toString());
                              }

                              // إنشاء نص التفاصيل مع التنسيق المطلوب
                              final details = controller.detailsMap[currentKey]!
                                  .map((detailId) {
                                ...
                                final element = controller.MAT_DES_M.firstWhere(
                                      (element) =>
                                  element.MDMID.toString() == detailId,
                                  orElse: () =>
                                      Mat_Des_M_Local(MDMID: 0, MDMNA_D: ''),
                                );
                                return element.MDMNA_D.toString();
                              }).map((
                                  detail) => '<$detail>') // إضافة العلامة <> حول كل تفصيل
  ...
  DisplayAddDetails(GETBMMID,GETBMDID) {
    Get.bottomSheet(
      StatefulBuilder(
        builder: (BuildContext context, StateSetter setState) {
          double width = MediaQuery.of(context).size.width;
          double height = MediaQuery.of(context).size.height;
          double padding = 0.02 * height;
              ...
              child: Align(
                ...
                child: ConstrainedBox(
                  constraints: BoxConstraints(
                    maxWidth: 600, // أقصى عرض للآيباد والشاشات الكبيرة
                  ),
                  child: Column(
                    children: [
                      Container(
                          ...
                          children: [
                            ...
                            Padding(
                              ...
                              child: Column(
                                children: [
                                  TextFormField(
                                    controller: controller.BMDINController,
                                  ...
                                  Container(
                                    ...
                                    child: TextButton(
                                      ...
                                      onPressed: () async {
                                        if(controller.BMDINController.text.trim().isNotEmpty){
                                          await UpdateBMDIN(
                                            controller.BMKID == 11 || controller.BMKID == 12
                                                ? 'BIF_MOV_D'
                                                : 'BIL_MOV_D',
                                            GETBMMID, // رقم الحركة الرئيسي
                                            GETBMDID, // رقم الحركة الفرعي
                                            controller.BMDINController.text, // التفاصيل
                                          );
                                          Fluttertoast.showToast(
                                              msg: 'StringMesSave'.tr,
      ...
      isScrollControlled: true, // مهم لدعم التمرير الكامل على الشاشات الصغيرة
    );
  }


  DisplayAddDetails3(GETBMMID,GETBMDID) {
    Get.bottomSheet(
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            Padding(
                              padding: EdgeInsets.only(
                                  right: 0.05 * width, left: 0.05 * width),
                              child: Column(
                                children: [
                                  TextFormField(
                                    controller: controller.BMDINController,
