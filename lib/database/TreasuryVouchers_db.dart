...
Future<List<Acc_Mov_M_Local>> GET_ACC_MOV_M_PRINT(
  ...
  return await ErrorHandlerService.run(() async {
    ...
    return list;
  },Err: 'GET_ACC_MOV_M_PRINT');
}

Future<int> DELETEACC_MOV_M(String GETAMKID, String GETAMMID) async {
  return await ErrorHandlerService.run(() async {
    var dbClient = await conn.database;
    return await dbClient!.delete('ACC_MOV_M', where: 'AMKID=$GETAMKID AND AMMID=$GETAMMID');
  },Err: 'DELETEACC_MOV_M');
}
...
Future<int> DeleteACC_MOV_D_ONE(String DELAMMID, String DELAMDID) async {
  return await ErrorHandlerService.run(() async {
    var dbClient = await conn.database;
    return await dbClient!.delete('ACC_MOV_D', where: 'AMMID=$DELAMMID AND AMDID=$DELAMDID');
  },Err: 'DeleteACC_MOV_D_ONE');
}
...
Future<int> UpdateStateACC_MOV_D(String TypeSync, String GETAMKID,
  ...
  return await ErrorHandlerService.run(() async {
    ...
    var result = await dbClient!.rawUpdate(SQL);
    return result;
  },Err: 'UpdateStateACC_MOV_D');
}

Future<List<Acc_Mov_D_Local>> GET_ROWN1(String GetAMKID, String GetAMMID) async {
  return await ErrorHandlerService.run(() async {
    var dbClient = await conn.database;
    String sql;
    sql =
